...
# Get all ICU stays.
icu = pd.read_csv(mimic_data_dir+'icustays.csv', usecols=['subject_id', 'hadm_id', 'stay_id', 'intime', 'outtime','first_careunit'])
...
adm_eth_icu = pd.read_csv(mimic_data_dir+'admissions.csv', usecols=['subject_id', 'race','admission_type','admission_location','insurance','language','marital_status'])
...
#Adding ethnicity data in 5 groups
...
icu['race'] = icu['race'].str.strip().str.lower()
#print(df_admission_unique['race'])

def get_race_sub_group(race):
    for group, races in ethnicity.items():
        if race in races:
            return group
    return 'Unknown'

icu['ethnicity'] = icu['race'].apply(get_race_sub_group)
...
ch['TABLE'] = 'chart'
#ch.to_csv('mimic_iv_ch.csv', index=False)
...
la['stay_id'] = np.nan
la['TABLE'] = 'lab'
#la.to_csv('mimic_iv_la.csv', index=False)
...
oe['TABLE'] = 'output'
#oe.to_csv('mimic_iv_oe.csv', index=False)
...
#oeitems.to_csv('mimic_iv_oeitemsinfo.csv', index=False)
...
#ie.to_csv('mimic_iv_ie.csv', index=False)
...
# Save data.
events.to_csv('mimic_iv_events_duett_april10.csv', index=False)
icu.to_csv('mimic_iv_icu_duett_april10.csv', index=False)
...
events = pd.read_csv('mimic_iv_events_duett_april10.csv', usecols=['hadm_id', 'stay_id', 'charttime', 'valuenum', 'TABLE', 'name'])
icu = pd.read_csv('mimic_iv_icu_duett_april10.csv')
...
# Get icustays which lasted for atleast 48 hours.
icu = icu.loc[(icu.outtime-icu.intime)>=pd.Timedelta(48,'h')]

# Get icustays with patient alive for atleast 48 hours.
...
icu = icu.loc[((icu.deathtime-icu.intime)>=pd.Timedelta(48,'h'))|icu.deathtime.isna()]

# Get icustays with aleast one event in first 48h.
icu = icu.loc[icu.stay_id.isin(events.loc[events.rel_charttime<48*60].stay_id)]
...
#Adding more categorical static variables as per the DuETT paper

categorical_vars = ['ethnicity', 'insurance', 'language', 'marital_status', 
                    'admission_location', 'admission_type', 'first_careunit']
for var in categorical_vars:
    # Map each unique category to a unique integer
    categories = icu_full[var].astype('category').cat.categories
    mapping = {category: code for code, category in enumerate(categories)}
    icu_full[var] = icu_full[var].map(mapping)

# Add these variables to the events DataFrame, similar to 'Gender' and 'Age'
for var in categorical_vars:
    data_var = icu_full[['ts_ind', var]]
    data_var['variable'] = var.lower()
    data_var.rename(columns={var: 'value'}, inplace=True)
    data_var['minute'] = 0  # Assuming these are static variables and do not change over time
    events = pd.concat((events, data_var), ignore_index=True)
...
icu.to_csv('mimic_iv_final_icu_48hr_admitted.csv', index=False)

events.to_csv('mimic_iv_final_events_with_all_data_for_duett.csv', index=False)
...
oc.to_csv('mimic_iv_mortality_label_for_ebtire_icu_admitted_pat.csv')


df = pd.read_csv('/home/anand/UHIF/UniHPF-master/mimic_iv_final_events_with_all_data_for_duett.csv')
pivoted_df = df.pivot_table(index=['ts_ind', 'stay_id', 'minute'], 
                            columns='variable', 
                            values='value', 
                            aggfunc='first').reset_index()

#pivoted_df.to_csv('mimic_iv_DueTT_data_pivot.csv', index=False)
pivoted_df.fillna(0, inplace=True)

icu_DuETT = pd.read_csv('/home/anand/UHIF/UniHPF-master/mimic_iv_final_icu_48hr_admitted.csv')
columns_to_keep = ['subject_id','stay_id', 'first_careunit','gender', 'anchor_age', 'insurance', 'language',
       'marital_status', 'ethnicity'
]
filtered_icu_DuETT = icu_DuETT[columns_to_keep]
filtered_icu_DuETT = filtered_icu_DuETT.drop_duplicates(keep='first')
categorical_columns = [
    'first_careunit', 'gender', 'insurance', 'language',
       'marital_status', 'ethnicity'
]
# Dictionary to hold the mapping from category to code
category_mappings = {}

# Convert categorical columns to numerical and store the mappings
for column in categorical_columns:
    filtered_icu_DuETT[column] = filtered_icu_DuETT[column].astype('category')
    category_mappings[column] = dict(enumerate(filtered_icu_DuETT[column].cat.categories))

# category_mappings now holds the mapping
print(category_mappings)

# You can also see the mapping for each category
for column, mapping in category_mappings.items():
    print(f'Mapping for {column}: {mapping}')

# Optionally, save the mappings to a file
with open('category_mappings.pickle', 'wb') as handle:
    pickle.dump(category_mappings, handle, protocol=pickle.HIGHEST_PROTOCOL)

# Convert columns to their numerical codes
for column in categorical_columns:
    filtered_icu_DuETT[column] = filtered_icu_DuETT[column].cat.codes

# Now df has all the categorical columns converted to numerical codes
# You can inspect the DataFrame to see the changes
#

# Optionally, save the updated DataFrame to a new CSV file
# df.to_csv('numerical_data.csv', index=False)
merged_df = pd.merge(pivoted_df, filtered_icu_DuETT, on='stay_id', how='left')
merged_df.to_csv('mimic_iv_DuETT_eveandicuwithstatic.csv', index=False)

#print(merged_df.head(1000))

#label = pd.read_csv('/home/anand/UHIF/UniHPF-master/mimic_iv_mortality_label_for_ebtire_icu_admitted_pat.csv')
#merged_final_df = pd.merge(final_df, label[['subject_id','in_hospital_mortality']], on='subject_id', how='left')
#merged_final_df.to_csv('mimic_iv_DueTT_final_with_label.csv', index=False)

merged_df = pd.read_csv('/home/anand/UHIF/UniHPF-master/mimic_iv_DuETT_eveandicuwithstatic.csv')
static_columns = ['stay_id', 'first_careunit','gender', 'anchor_age', 'insurance', 'language',
       'marital_status', 'ethnicity'
]

# Getting unique static rows for each stay_id
static_rows = merged_df[static_columns].drop_duplicates()

# Check if each stay_id has a minute=0 row, if not create one
minute_zero_rows = []
for index, row in static_rows.iterrows():
    if not ((merged_df['stay_id'] == row['stay_id']) & (merged_df['minute'] == 0)).any():
        new_row = row.to_dict()
        new_row['minute'] = 0
        # Setting dynamic column values to NaN
        for col in merged_df.columns:
            if col not in static_columns and col != 'minute':
                new_row[col] = np.nan
        minute_zero_rows.append(new_row)

# Convert the list of dicts to DataFrame
minute_zero_df = pd.DataFrame(minute_zero_rows)

# Concatenate the new rows with the original DataFrame
final_df = pd.concat([merged_df, minute_zero_df], ignore_index=True)

# Sort by stay_id and minute
final_df.sort_values(by=['stay_id', 'minute'], inplace=True)

# Reset index
final_df.reset_index(drop=True, inplace=True)
final_df.to_csv('mimic_iv_DueTT_final_without_label.csv', index=False)
label = pd.read_csv('/home/anand/UHIF/UniHPF-master/mimic_iv_mortality_label_for_ebtire_icu_admitted_pat.csv')
merged_final_df = pd.merge(final_df, label[['subject_id','in_hospital_mortality']], on='subject_id', how='left')
merged_final_df.to_csv('mimic_iv_DueTT_final_with_label.csv', index=False)